    ...
    >
      ...
      >
        ...
        <div class="flex-1 flex flex-col min-h-0">
          ...
          <div
            v-if="!editing"
            class="flex-shrink-0 px-4 py-3 border-b border-border space-y-2"
          >
            <!-- Row 1: date · author + edit pencil -->
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                ...
                <template v-if="authorName">
                  ...
                  <span
                    :class="
                      isFormerMember ? 'text-muted-foreground/40' : ''
                    "
                    >{{ authorName }}</span
                  >
                </template>
            ...
            >
              <span
                class="text-[10px] font-semibold tracking-[.08em] uppercase"
              ...
              >
                <div
                  class="w-5 h-5 rounded-full overflow-hidden bg-secondary flex-shrink-0 flex items-center justify-center text-[8px] font-bold text-foreground"
                >
                  <img
          ...
          <div
            v-else
            class="flex-shrink-0 px-4 py-3 border-b border-border"
          >
            <!-- Milestone -->
            ...
            >
              <p
                class="text-[10px] font-semibold text-muted-foreground uppercase tracking-[.12em] mb-1.5"
              >
                {{ t("modal.whoIsIn") }}
              </p>
                    ...
                    <template v-if="editingCommentId !== c.id">
                      <div class="relative">
                        <div class="bg-secondary rounded-2xl rounded-tl-sm px-3 py-2">
                          ...
                          >
                          <span class="text-[13px] text-foreground leading-snug">{{
                            c.body
                          }}</span>
                        </div>
                        <button
...
const childAges = computed(() => {
  ...
  return (memory.value.memory_children ?? [])
    .map((mc) => ({
      ...
      age: computeBabyAge(mc.childprofile.date_of_birth, memory.value!.memory_date),
    }))
...
const authorName = computed(() => {
  ...
  if (isFormerMember.value && memory.value?.former_owner_name)
    return memory.value.former_owner_name;
  return null;
});
...
function startEditing() {
  ...
  editChildIds.value = (memory.value?.memory_children ?? []).map(
    (mc) => mc.child_id,
  );
  editMemberIds.value = (memory.value?.memory_members ?? []).map(
    (mm) => mm.user_id,
  );
  editing.value = true;
...
async function saveEdit() {
  ...
  try {
    const [{ memory: updated }] = await Promise.all([
      $fetch<{ memory: { id: string; note: string | null; milestone_label: string | null } }>(
...
// Self profile for comment input avatar
...
// ── Open/close when memory prop changes ───────────────────
watch(
  () => props.memory,
  async (mem) => {
    if (mem) {
      localReactions.value = [...(mem.memoryreaction ?? [])] as Reaction[];
      comments.value = [];
      commentDraft.value = "";
