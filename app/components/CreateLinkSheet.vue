<template>
  <Transition name="fade">
    <div v-if="open" class="fixed inset-0 z-[55] bg-black/40 backdrop-blur-sm" @click="$emit('close')" />
  </Transition>

  <Transition name="slide-up">
    <div v-if="open" class="fixed inset-x-0 bottom-0 z-[60] bg-card border-t border-border rounded-t-[24px] shadow-2xl max-h-[90vh] flex flex-col">
      ...
      <div class="flex items-center justify-between px-5 py-3 flex-shrink-0">
        ...
        <button type="button" @click="$emit('close')" :aria-label="t('viewerLink.close')"
      ...
      <div class="flex gap-1 mx-5 mb-3 p-1 bg-secondary rounded-[12px] flex-shrink-0">
        <button
          v-for="mode in mainModes" :key="mode.value"
          type="button"
          @click="selectedMode = mode.value"
          class="flex-1 h-8 rounded-[10px] text-xs font-semibold transition-all"
          :class="selectedMode === mode.value ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
        >{{ mode.label }}</button>
      </div>

      <!-- Selection status bar (Custom mode only, fixed above scroll) -->
      <div v-if="selectedMode === 'custom'" class="flex-shrink-0 flex items-center justify-between px-5 pb-2.5 border-b border-border/40">
        <div class="flex items-center gap-3 min-h-[20px]">
          <span v-if="selectedMemoryIds.size > 0" class="text-sm font-semibold text-foreground">
        ...
        <div v-if="yearsLoading || isAnyYearLoading" class="flex items-center gap-1.5 text-[11px] text-muted-foreground">
      ...
      <div class="flex-1 overflow-y-auto pb-4">
        ...
        <div v-if="selectedMode === 'full'" class="px-5 py-4">
          <p class="text-sm text-muted-foreground leading-relaxed">{{ t('viewerLink.fullModeDescription') }}</p>
        </div>
        ...
        <template v-else>
          ...
          <div v-else>
            <div v-for="group in yearGroups" :key="group.year">
              ...
              <!-- Year header (sticky) -->
              <div class="sticky top-0 bg-card z-10 flex items-center justify-between px-5 pt-3 pb-2 border-b border-border/20">
                <div class="flex items-center gap-2.5">
                  <button
                    type="button"
                    :disabled="!group.loaded"
                    @click="group.loaded && toggleYear(group.year)"
              ...
              <div v-if="group.loaded" class="px-5">
                <div v-for="mg in getMonthGroups(group)" :key="mg.month" class="mt-3 mb-2">

                  <!-- Month header -->
                  <div class="flex items-center gap-1.5 mb-1.5">
                    <button
                      type="button"
                      @click="toggleMonth(group.year, mg.month)"
                    ...
                    <span class="text-[11px] font-semibold text-foreground/60">{{ monthName(mg.month) }}</span>
                    <span class="text-[10px] text-muted-foreground tabular-nums">{{ mg.memories.length }}</span>
                  </div>
...
const selectionStats = computed(() => {
  ...
  for (const g of yearGroups.value) {
    ...
    for (const m of g.memories) {
      const mo = new Date(m.memory_date + 'T12:00:00').getMonth() + 1
      const k = `${g.year}-${mo}`
      if (!byYM.has(k)) byYM.set(k, { sel: 0, total: 0 })
      const ms = byYM.get(k)!
      ms.total++
      if (selectedMemoryIds.value.has(m.id)) { ms.sel++; ySel++ }
    }
...
function getMonthGroups(group: YearGroup): MonthGroup[] {
  ...
  for (const m of group.memories) {
    const month = new Date(m.memory_date + 'T12:00:00').getMonth() + 1
...
function toggleMonth(year: number, month: number) {
  ...
  const monthMems = group.memories.filter(
    m => new Date(m.memory_date + 'T12:00:00').getMonth() + 1 === month
  )
  const state = isMonthSelected(year, month)
  const s = new Set(selectedMemoryIds.value)
  if (state === 'full') {
  ...
  try {
    await $fetch(`/api/circles/${props.circleId}/viewer-links`, {
      ...
      body: {
        ...
        memoryIds: selectedMode.value === 'custom' ? [...selectedMemoryIds.value] : undefined,
      },
    })
    emit('created')
    emit('close')
