...
test.describe('Multi-circle support', () => {
  ...
  test('no ?circle= param shows the first circle by default', async ({
    page,
  }) => {
    await page.goto('/timeline')
    // Wait for page to render the circle name
    await expect(page.getByRole('button', { name: CIRCLE_1.name })).toBeVisible(
      { timeout: 15_000 },
    )
  })
  ...
  test('unknown ?circle= param falls back to the first circle', async ({
    page,
  }) => {
  ...
  // ── Circle switcher UI ──────────────────────────────────────────────────
  test('clicking the circle name opens the switcher modal', async ({
    page,
  }) => {
    await page.goto('/timeline')
    await page
      .getByRole('button', { name: CIRCLE_1.name })
  ...
  test('switcher modal lists all circles', async ({ page }) => {
    ...
    await expect(
      page.getByRole('listitem').filter({ hasText: CIRCLE_1.name }),
    ).toBeVisible()
    await expect(
      page.getByRole('listitem').filter({ hasText: CIRCLE_2.name }),
    ).toBeVisible()
  })

  test('clicking a circle in the switcher updates the ?circle= URL param', async ({
    page,
  }) => {
    await page.goto('/timeline')
    await page
    ...
    await expect(page).toHaveURL(new RegExp(`circle=${CIRCLE_2.id}`), {
      timeout: 5_000,
    })
  })

  test('after switching circles the new circle name appears in the header', async ({
    page,
  }) => {
    await page.goto('/timeline')
    await page
  ...
  test('"Start new circle" button in the switcher navigates to /onboarding', async ({
    ...
    await page
      .getByRole('button', { name: /start.*circle|new circle/i })
      .click()
  ...
  test('clicking the backdrop closes the switcher', async ({ page }) => {
    ...
    await page
      .locator('.absolute.inset-0.bg-black\\/40')
      .click({ position: { x: 10, y: 10 } })

    await expect(page.getByText('Your circles')).not.toBeVisible({
      timeout: 3_000,
    })
  })
})
