...
test.describe('Onboarding flow', () => {
  ...
  test.describe('Circle type picker (/onboarding)', () => {
    ...
    test('Continue button is disabled before a type is selected', async ({
      page,
    }) => {
      await expect(
    ...
    test('selecting a type and clicking Continue navigates to the name step', async ({
      page,
    }) => {
      await page
        .locator('button')
        .filter({ hasText: 'New parents' })
        .first()
        .click()
  ...
  test.describe('Circle name step (/onboarding/name)', () => {
    ...
    test('shows a name input with a type-specific placeholder', async ({
      page,
    }) => {
    ...
    test('non-solo circle navigates to the invite step after creation', async ({
      page,
    }) => {
    ...
    test('solo circle skips invite and navigates to the timeline', async ({
      page,
    }) => {
      ...
      await expect(page).toHaveURL(
        new RegExp(`timeline.*circle=${TEST_CIRCLE_ID}`),
      )
    })

    test('shows an error message when circle creation fails', async ({
      page,
    }) => {
      await page.route('**/api/circles/create**', (route) =>
        route.fulfill({
  ...
  test.describe('Invite step (/onboarding/invite)', () => {
    ...
    test('Send invite button is disabled when email is empty', async ({
      page,
    }) => {
      await expect(
        page.getByRole('button', { name: 'Send invite' }),
    ...
    test('sending a valid invite shows a success confirmation', async ({
      page,
    }) => {
    ...
    test('after sending an invite the skip button changes to Continue', async ({
      page,
    }) => {
      await page.route('**/api/circles/invite**', (route) =>
        route.fulfill({
    ...
    test('skipping invite navigates to the timeline with the new circle', async ({
      page,
    }) => {
