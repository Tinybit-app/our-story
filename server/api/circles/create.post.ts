...
export default defineEventHandler(async (event) => {
  ...
  if (error || !circle) {
    ...
    throw createError({ statusCode: 500, message: "Failed to create circle. Please try again." })
  }
  ...
  if (memberError) {
    console.error("[create-circle] circlemember insert failed:", memberError.message)
