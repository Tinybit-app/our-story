...
export default defineEventHandler(async (event) => {
  ...
  if (!circle.deleted_at)
    throw createError({
      statusCode: 409,
      message: 'Circle is not scheduled for deletion.',
    })
  ...
  if (daysSinceDeletion >= 30) {
    throw createError({
      statusCode: 410,
      message:
        'The 30-day recovery window has passed. This circle cannot be restored.',
    })
  }
  ...
  if (error) {
    ...
    throw createError({
      statusCode: 500,
      message: 'Failed to restore circle. Please try again.',
    })
  }
