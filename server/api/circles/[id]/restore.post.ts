...
export default defineEventHandler(async (event) => {
  ...
  if (!circle.deleted_at) throw createError({ statusCode: 409, message: "Circle is not scheduled for deletion." })
  if (circle.deletion_initiated_by !== user.sub) {
    throw createError({ statusCode: 403, message: "Only the owner who initiated deletion can restore this circle." })
  }
