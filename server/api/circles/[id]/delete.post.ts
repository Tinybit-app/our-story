...
export default defineEventHandler(async (event) => {
  ...
  if (circle.deleted_at) throw createError({ statusCode: 409, message: "Circle is already scheduled for deletion." })

  // Validate the typed confirmation matches the circle name
  if (confirmName.trim().toLowerCase() !== circle.name.trim().toLowerCase()) {
    throw createError({ statusCode: 422, message: "Circle name does not match. Please type it exactly." })
  }
