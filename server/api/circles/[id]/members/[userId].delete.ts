...
export default defineEventHandler(async (event) => {
  ...
  if (keepContent && memoryIds.length > 0) {
    ...
    const formerName = [memberProfile?.first_name, memberProfile?.last_name]
      .filter(Boolean).join(" ") || null

    await supabase
      .from("memory")
      .update({ owner_user_id: null, former_owner_name: formerName, former_owner_user_id: targetUserId })
      .eq("owner_user_id", targetUserId)
      .eq("circle_id", circleId)
  ...
  try {
    const [{ data: removedUser }, { data: circle }, { data: remover }] = await Promise.all([
      (supabase as any).from("user").select("email, first_name, locale").eq("id", targetUserId).maybeSingle(),
    ...
    if (removedUser?.email) {
      const removerName = [remover?.first_name, remover?.last_name].filter(Boolean).join(" ") || "Someone"
