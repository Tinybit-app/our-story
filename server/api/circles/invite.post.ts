...
export default defineEventHandler(async (event) => {
  ...
  // Expire any existing pending invites for this email before issuing a fresh
  // one. This lets owners resend at any time — the old link is invalidated, a
  // new 7-day window starts. Run this BEFORE the 10-invite cap check so a
  // resend to the same email doesn't get falsely blocked when the circle is
  // at the cap (the row about to be expired would otherwise inflate the count).
  await supabase
    .from('circleinvite')
  ...
  // Check max pending invites (10 per circle) — now counts truly-still-pending
  // rows since any prior pending row for this same email was just expired above.
