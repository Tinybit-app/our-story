...
export default defineEventHandler(async (event) => {
  ...
  if (userIds.length > 0) {
    ...
    if (!userIds.every((id) => validSet.has(id))) {
      throw createError({
        statusCode: 400,
        message: 'One or more user IDs are not circle members.',
      })
    }
  }
  ...
  const existingUserIds = new Set(
    (existingTags ?? []).map((t: any) => t.user_id),
  )
  const newlyTaggedIds = userIds.filter(
    (id) => !existingUserIds.has(id) && id !== user.sub,
  )
  ...
  if (deleteError) {
    ...
    throw createError({
      statusCode: 500,
      message: 'Failed to update member tags.',
    })
  }
  ...
  if (userIds.length > 0) {
    const { error: insertError } = await (supabase as any)
      .from('memory_members')
      .insert(
        userIds.map((userId) => ({ memory_id: memoryId, user_id: userId })),
      )
...
async function sendTagNotifications(
  ...
  const taggerName =
    [taggerResult.data?.first_name, taggerResult.data?.last_name]
      .filter(Boolean)
      .join(' ') || 'Someone'
