          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <template v-if="revokingId === link.id">
                <p class="flex-1 text-xs text-muted-foreground text-center">{{ t('viewerLink.revokeConfirmBody') }}</p>
                <button
...
<template>
  ...
  <Transition name="slide-up">
    ...
    >
      ...
      <div class="flex-shrink-0 px-5 pt-2 pb-3 flex items-center justify-between border-b border-border">
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-5">
        <!-- Mode selector -->
        <div>
          <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Mode</p>
          <div class="flex gap-2">
            <button
              v-for="m in modes"
              :key="m.value"
              @click="selectedMode = m.value"
              class="flex-1 py-2 rounded-[10px] text-xs font-semibold border transition-colors"
        ...
        <div v-if="selectedMode === 'date_range'" class="flex flex-col gap-3">
          <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Date range</p>
          <!-- Quick picks -->
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :class="isYearSelected(year) ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground'"
            >
              {{ year }}
            </button>
          </div>
          ...
          <div class="flex gap-2">
            <div class="flex-1">
              ...
              <input v-model="dateFrom" type="date" class="w-full border border-border rounded-xl px-3 py-2 text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div class="flex-1">
              <label class="text-xs text-muted-foreground block mb-1">To</label>
              <input v-model="dateTo" type="date" class="w-full border border-border rounded-xl px-3 py-2 text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedMode === 'selection'" class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            ...
            <span v-if="selectedMemoryIds.size > 0" class="text-xs text-accent font-semibold">
              {{ t('viewerLink.selectedCount', { count: selectedMemoryIds.size }) }}
            </span>
          </div>
          <div v-if="memoriesLoading" class="py-6 text-center text-sm text-muted-foreground">Loading…</div>
          <div v-else class="grid grid-cols-3 gap-1.5">
            <button
              v-for="memory in allMemories"
              :key="memory.id"
              @click="toggleMemory(memory.id)"
              class="relative aspect-square rounded-[8px] overflow-hidden border-2 transition-all"
        ...
        <!-- Label field -->
        <div>
          <label class="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-2">Label</label>
          <input
            v-model="label"
            type="text"
            :placeholder="t('viewerLink.labelPlaceholder')"
            class="w-full border border-border rounded-xl px-4 py-2.5 text-sm bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          />
        </div>
      </div>

      <!-- Create button -->
      <div class="flex-shrink-0 px-5 pb-8 pt-3 border-t border-border">
        <button
          @click="handleCreate"
        ...
        <Transition name="fade">
          ...
          >
            ...
            <button
              ...
              class="block mx-auto mt-2 text-xs text-muted-foreground hover:text-foreground"
            >
              Dismiss
            </button>
          </div>
...
test.describe("viewer link management", () => {
  ...
  test("create full-timeline link → copy URL → /view loads", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /create a link/i }).last().click()
