  ...
  for (const cand of candidates) {
    try {
      ...
      const { data: members } = await supabase
        ...
        .eq("circle_id", cand.circle_id)
        .in("role", ["owner", "admin"])
      const recipients = (members ?? []).filter((m: any) => !m.user.deletion_requested_at && m.user.email)
      ...
      for (const r of recipients as any[]) {
        ...
        const { data: prefs } = await supabase
          ...
          .select("circle_muted, milestone_nudges_enabled, push_enabled, email_digest_frequency, quiet_hours_start, quiet_hours_end")
          .eq("user_id", userId)
          .eq("circle_id", cand.circle_id)
          .maybeSingle()
        if (prefs?.circle_muted) { skipped++; continue }
        if (prefs && prefs.milestone_nudges_enabled === false) { skipped++; continue }
        ...
        // Fallback to email
        if (prefs?.email_digest_frequency === "off") { skipped++; continue }
        ...
        const { subject, html } = cand.scope_type === "child"
