...
describe('POST /api/invites/[token]/accept — deleted circle guard', () => {
  ...
  it('rejects when the circle has been soft-deleted', () => {
    expect(
      acceptResult({
        inviteStatus: 'pending',
        circleDeletedAt: '2026-04-18T00:00:00Z',
        inviteExpired: false,
      }),
    ).toBe('invite_circle_deleted')
  })
